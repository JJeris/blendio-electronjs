...
export async function insertBlendFile(_, filePath) {
    try { // C (3.d.) try
        let entry = new ProjectFile({ // A (1.a.) let entry =; B (2.b.) new ProjectFile;
...
export async function insertAndRefreshBlendFiles(_) {
    try { // C (3.d.) try
        ...
        } else if (platform === 'darwin') { // C (3.b.) else; C (3.a.) platform === 'darwin'
        ...
        let directoryEntries = fs.readdirSync(blenderFoundationDirectory, { withFileTypes: true }).filter(entry => entry.isDirectory()); // A (1.a.) let directoryEntries =; B (2.a.) .readdirSync()
        for (let entry of directoryEntries) { // A (1.a.) let entry =;
            let seriesName = entry.name; // A (1.a.) let seriesName =; 
            let recentFilesTxtPath = path.join(blenderFoundationDirectory, seriesName, 'config', 'recent-files.txt'); // A (1.a.) let recentFilesTxtPath =; B (2.a.) .join()
            if (!fs.existsSync(recentFilesTxtPath)) { // C (3.a.) fs.existsSync() != true; B (2.a.) .existsSync()
            ...
            for (let line of recentFilesTxtContent.split(/\r?\n/)) { // A (1.a.) let line =; B (2.a.) .split()
                ...
                if (filePath.length === 0) { // C (3.a.) filePath.length === 0
                    continue; // B (2.c.) continue
                }
                if (!fs.existsSync(filePath)) { // C (3.a.) fs.existsSync() != true; B (2.a.) .existsSync()
                    let currentEntries = projectFileRepo.fetch(null, null, filePath); // A (1.a.) let currentEntries =; B (2.a.) projectFileRepo.fetch()
                    if (currentEntries && currentEntries.length > 0) { // C (3.a.) currentEntries == truthy; C (3.a.) currentEntries.length > 0
                ...
                if (existingEntries.length === 0) { // C (3.a.) existingEntries.length === 0
                    let newProjectFileEntry = new ProjectFile({ // A (1.a.) let newProjectFileEntry =; B (2.b.) new ProjectFile;
                        id: uuidv4(), // B (2.a.) uuidv4();
                        file_path: filePath,
                        file_name: path.basename(filePath), // B (2.a.) .basename()
                    ...
                    if (!associatedSeriesJson.includes(seriesName)) { // C (3.a.) associatedSeriesJson.includes() != true;  B (2.a.) .includes()
        ...
        for (let entry of currentEntries) { // A (1.a.) let entry =;
            if (!fs.existsSync(entry.file_path)) { // C (3.a.) fs.existsSync() != true; B (2.a.) .existsSync()
...
export async function deleteBlendFile(_, id) {
    try { // C (3.d.) try
        let confirmation = await showAskNotification("Are you sure you want to delete this .blend file?", "warning"); // A (1.a.) let confirmation =; B (2.a.) showAskNotification()
        ...
        if (!projectFileList || projectFileList.length === 0) { // C (3.a.) projectFileList != truthy; C (3.a.) projectFileList.length === 0
...
export async function openBlendFile(_, id, installedBlenderVersionId, launchArgumentsId = null, pythonScriptId = null) {
    try { // C (3.d.) try
        ...
        let installedBlenderVersionList = installedBlenderVersionRepo.fetch(installedBlenderVersionId, null, null); // A (1.a.) let installedBlenderVersionList =; B (2.a.) installedBlenderVersionRepo.fetch()
...
export async function createNewProjectFile(_, installedBlenderVersionId, fileName) {
    try { // C (3.d.) try
        ...
        if (!directoryPath) { // C (3.a.) directoryPath != truthy
            return; // B (2.c.) priekšlaicīgs return
        }
        if (!fileName.endsWith('.blend')) { // C (3.a.) fileName.endsWith('.blend') != truthy; B (2.a.) .endsWith()  
        ...
        let pythonCodeExpression = // A (1.a.) let pythonCodeExpression =;
`
${IMPORT_BPY}
...
`;
        ...
        await launchExecutable(entry.executable_file_path, ["--background", "--python-expr", pythonCodeExpression]); // B (2.a.) launchExecutable()
...
export async function createProjectFileArchiveFile(_, id) {
    try { // C (3.d.) try
        ...
        if (!entryList || entryList.length === 0) { // C (3.a.) entryList != truthy; C (3.a.) entryList.length === 0
            throw "Failed to fetch project file by ID"; // B (2.c.) throw
        }

...
export async function insertPythonScript(_) {
    try { // C (3.d.) try
        ...
        if (!filePath) { // C (3.a.) filePath != truthy
        ...
        let entry = new PythonScript({ // A (1.a.) let entry.accessed =; B (2.b.) new PythonScript;
...
export async function deletePythonScript(_, id) {
    try { // C (3.d.) try
        let confirmation = await showAskNotification("Are you sure you want to delete this python script entry?", "warning"); // A (1.a.) let confirmation =; B (2.a.) showAskNotification()

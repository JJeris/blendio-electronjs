...
export function instancePopupWindow(_, label, title, urlPath = '/') {
    try { // C (3.d.) try
        let popupWindow = new BrowserWindow({ // A (1.a.) let popupWindow =; B (2.b.) new BrowserWindow
            width: 600,
            height: 400,
            title,
            show: false,
        ...
        popupWindow.on('ready-to-show', () => { // A (1.e.) .on()
            popupWindow.show(); // A (1.e.) .show()
        });
        popupWindow.webContents.setWindowOpenHandler((details) => { // A (1.e.) .setWindowOpenHandler(); B (2.a.) (details) => {...}
            shell.openExternal(details.url); // A (1.e.) .openExternal()
            return { action: 'deny' }; // B (2.c.) priekšlaicīgs return
        });
        if (is.dev && process.env['ELECTRON_RENDERER_URL']) { // C (3.a.) is.dev == true; C (3.a.) process.env['ELECTRON_RENDERER_URL'] == true
            popupWindow.loadURL(`${process.env['ELECTRON_RENDERER_URL']}?route=/${urlPath}`); // A (1.e.) .loadURL()
        } else { // C (3.b.) else;
            popupWindow.loadFile(path.join(__dirname, '../renderer/index.html'), { // A (1.e.) .loadFile(); B (2.a.) .join()
                query: { route: `/${urlPath}` }
            });
        }
        return;
...
export async function showOkNotification(message, kind) {
    await dialog.showMessageBox(global.mainWindow, { // B (2.b.) .showMessageBox()
...
export async function showAskNotification(message, kind) {
    let result = await dialog.showMessageBox(global.mainWindow, { // A (1.a.) let result =; B (2.b.) .showMessageBox()
      type: kind, // 'info', 'warning', or 'error'
      message: message,
      buttons: ['Yes', 'No'],
      defaultId: 0,
...
export async function openInFileExplorer(filePath) {
    try { // C (3.d.) try
        ...
        if (!parentDirectory) { // C (3.a.) parentDirectory != truthy
            throw "Failed to find parent directory in file path"; // B (2.c.) throw
        }
        let command;
        let args;
        switch (process.platform) { // C (3.b.) switch
...
export async function getFileFromFileExplorer() {
    try { // C (3.d.) try
        let result = await dialog.showOpenDialog(BrowserWindow.getFocusedWindow(), { // A (1.a.) let result =; B (2.a.) .showOpenDialog(); B (2.a.) .getFocusedWindow()
            title: 'Select File',
            properties: ['openFile'],
            filters: [{ name: 'Python Files', extensions: ['py'] }]
        });
        if (result.canceled || result.filePaths.length === 0) { // C (3.a.) result.canceled == true; C (3.a.) result.filePaths.length === 0
            return null; // B (2.c.) priekšlaicīgs return
        }
...
export async function getDirectoryFromFileExplorer() {
    try { // C (3.d.) try
        let result = await dialog.showOpenDialog(global.mainWindow, { // A (1.a.) let result =; B (2.a.) .showOpenDialog();
            title: 'Select Folder',
